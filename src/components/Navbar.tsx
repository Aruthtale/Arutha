  ...
  return (
    <>
      ...
      <nav className={cn(
        ...
        <div className="flex flex-col items-center w-full px-4 xl:px-6">
          {/* Logo Section */}
          <button
          ...
          {session && (
            <div className="flex flex-col gap-3 w-full">
              ...
              <SidebarPill active={currentPage === 'SOUL_GUARD'} onClick={() => onNavigate('SOUL_GUARD')} icon={<Heart className="w-5 h-5 text-karma" />} label="Soul Guard" />
              <SidebarPill active={currentPage === 'SETTINGS'} onClick={() => onNavigate('SETTINGS')} icon={<Settings className="w-5 h-5" />} label="Settings" />
              {isAdmin(session.user.email) && (
                <SidebarPill active={currentPage === 'ADMIN'} onClick={() => onNavigate('ADMIN')} icon={<ShieldAlert className="w-5 h-5 text-jiwa" />} label="Admin" />
              )}
            </div>
          )}
        </div>
        ...
        {session && stats && (
          <div className="px-4 xl:px-6 space-y-6">
            <div className="glass-panel p-4 xl:p-5 border border-white/10 bg-gradient-to-br from-jiwa/20 via-transparent to-ilmu/10 text-center relative overflow-hidden group rounded-2xl">
              ...
              <div className="xl:hidden flex justify-center py-1">
                 <div className="w-6 h-6 rounded-full bg-jiwa/20 flex items-center justify-center border border-jiwa/40">
                    <div className="w-2 h-2 rounded-full bg-jiwa animate-pulse shadow-[0_0_10px_#A855F7]" />
                 </div>
              </div>
